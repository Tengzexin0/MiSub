...
const handleClickOutside = (event) => {
  if (
    ...
    !nodesMoreMenuRef.value.contains(event.target)
  ) {
  ...
  if (
    ...
    !subsMoreMenuRef.value.contains(event.target)
  ) {
...
const handleSave = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      // 显示服务器返回的具体错误信息
      const errorMessage =
        result.message || result.error || "保存失败，请稍后重试";
...
const handleBulkImport = (importText) => {
  ...
  for (const line of lines) {
    ...
    if (/^https?:\/\//.test(line)) {
      ...
      /^(ss|ssr|vmess|vless|trojan|hysteria2?|hy|hy2|tuic|anytls|socks5):\/\//.test(
        line
      )
    ) {
  ...
  showToast(
    `成功导入 ${newSubs.length} 条订阅和 ${newNodes.length} 个手动节点，请点击保存`,
    "success"
  );
};
...
const formattedTotalRemainingTraffic = computed(() =>
  formatBytes(totalRemainingTraffic.value)
);
</script>
...
<template>
  ...
  <div v-else class="w-full max-w-screen-xl mx-auto p-4 sm:p-6 lg:p-8">
    <div class="flex justify-between items-center mb-8">
      ...
      <div class="flex items-center gap-2">
        <button
          ...
          class="text-sm font-semibold px-4 py-2 rounded-lg text-indigo-600 dark:text-indigo-400 border-2 border-indigo-500/50 hover:bg-indigo-500/10 transition-colors"
        >
          批量导入
        </button>
      </div>
    ...
    <Transition name="slide-fade">
      <div
        v-if="dirty"
        class="p-3 mb-6 rounded-lg bg-indigo-600/10 dark:bg-indigo-500/20 ring-1 ring-inset ring-indigo-600/20 flex items-center justify-between"
      >
      ...
      <div class="lg:col-span-2 space-y-12">
        <div>
          ...
          >
            ...
            <div
              class="flex items-center gap-2 w-full sm:w-auto justify-end sm:justify-start"
            >
              <button
                @click="handleAddSubscription"
                class="text-sm font-semibold px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white transition-colors shadow-sm flex-shrink-0"
              >
                新增
              </button>
              <div class="relative flex-shrink-0" ref="subsMoreMenuRef">
                ...
                <Transition name="slide-fade-sm">
                  <div
                    ...
                    class="absolute right-0 mt-2 w-36 bg-white dark:bg-gray-800 rounded-lg shadow-xl z-10 ring-1 ring-black ring-opacity-5"
                  >
                    <button
                      v-if="!isSortingSubs"
                      @click="
          ...
          <div v-if="subscriptions.length > 0">
              ...
              <template #item="{ element: subscription }">
                <div class="cursor-move">
                  <Card
                    :misub="subscription"
                    @delete="
                      handleDeleteSubscriptionWithCleanup(subscription.id)
                    "
                    @change="markDirty"
            ...
            <div
              v-if="subsTotalPages > 1 && !isSortingSubs"
              class="flex justify-center items-center space-x-4 mt-8 text-sm font-medium"
            >
              <button
                @click="changeSubsPage(subsCurrentPage - 1)"
              ...
              <span class="text-gray-500 dark:text-gray-400"
                >第 {{ subsCurrentPage }} / {{ subsTotalPages }} 页</span
              >
              <button
                @click="changeSubsPage(subsCurrentPage + 1)"
          ...
          <div
            v-else
            class="text-center py-12 text-gray-500 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <div>
          ...
          >
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
              ...
              <div
                class="p-0.5 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center flex-shrink-0"
              >
                <button
                  @click="setViewMode('card')"
                  class="p-1 rounded-md transition-colors"
                  :class="
                    manualNodeViewMode === 'card'
              ...
              <div class="relative flex-shrink-0" ref="nodesMoreMenuRef">
                ...
                <Transition name="slide-fade-sm">
                  ...
                  >
                    <button
                      @click="
                        showSubscriptionImportModal = true;
          ...
          <div v-if="manualNodes.length > 0">
            ...
            <div v-if="manualNodeViewMode === 'list'" class="space-y-2">
              <ManualNodeList
                ...
                :node="node"
                :index="
                  (manualNodesCurrentPage - 1) * manualNodesPerPage + index + 1
                "
                @edit="handleEditNode(node.id)"
                @delete="handleDeleteNodeWithCleanup(node.id)"
              />
            ...
            <div
              v-if="manualNodesTotalPages > 1 && !isSortingNodes"
              class="flex justify-center items-center space-x-4 mt-8 text-sm font-medium"
            >
              <button
                @click="changeManualNodesPage(manualNodesCurrentPage - 1)"
                :disabled="manualNodesCurrentPage === 1"
                class="px-3 py-1 rounded-md disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700"
              >
                &laquo; 上一页
              </button>
              <span class="text-gray-500 dark:text-gray-400"
                >第 {{ manualNodesCurrentPage }} /
                {{ manualNodesTotalPages }} 页</span
              >
              <button
                @click="changeManualNodesPage(manualNodesCurrentPage + 1)"
      ...
      <div class="lg:col-span-1 space-y-8">
        ...
        <div>
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-2">
              <button
                @click="showDeleteProfilesModal = true"
                class="hidden md:inline-flex text-sm font-medium px-3 py-1.5 rounded-lg text-red-500 border-2 border-red-500/60 hover:bg-red-500 hover:text-white dark:text-red-400 dark:border-red-400/60 dark:hover:bg-red-400 dark:hover:text-white transition-all"
              >
                清空
              </button>
              <button
                @click="handleAddProfile"
                class="text-sm font-semibold px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white transition-colors shadow-sm"
              >
                新增
              </button>
              <div
                class="relative md:hidden"
                v-on:mouseleave="showProfilesMoreMenu = false"
              >
                <button
                ...
                <Transition name="slide-fade-sm">
                  <div
                    ...
                    class="absolute right-0 mt-2 w-32 bg-white dark:bg-gray-800 rounded-lg shadow-xl z-10 ring-1 ring-black ring-opacity-5"
                  >
                    <button
                      @click="
                        showDeleteProfilesModal = true;
  ...
  <Modal
    ...
    ><template #title
      ><h3 class="text-lg font-bold text-red-500">确认清空订阅</h3></template
    ><template #body
  ...
  <Modal
    ...
    ><template #title
      ><h3 class="text-lg font-bold text-red-500">确认清空节点</h3></template
    ><template #body
      ><p class="text-sm text-gray-400">
  ...
  <Modal
    ...
    ><template #title
      ><h3 class="text-lg font-bold text-red-500">确认清空订阅组</h3></template
    ><template #body
      ><p class="text-sm text-gray-400">
